...
class UberAccessibilityService : AccessibilityService() {
    ...
    private fun loadConfig(): AppConfig {
        ...
        val minDistance = prefs.getFloat("min_customer_distance", 1.0f).toDouble()
        val maxDistance = prefs.getFloat("max_customer_distance", 3.0f).toDouble()
        
        return AppConfig(
            filterSettings = FilterSettings(
                mode = mode,
                minCustomerDistance = minDistance,
                maxCustomerDistance = maxDistance
            ),
