...
class FilterEngine(private val settings: FilterSettings) {
    ...
    fun isEligible(offer: UberOffer): FilterResult {
        ...
        // 조건1: 출발지 pickupKeywords 중 하나 AND 도착지 공항 키워드
        val condition1 = settings.pickupKeywords.any { offer.pickupLocation.contains(it, ignoreCase = true) } &&
            settings.airportKeywords.any { offer.dropoffLocation.contains(it, ignoreCase = true) }

        // 조건2: 출발지 공항 키워드 (도착지 무관, 모든 콜)
        val condition2 = settings.airportKeywords.any { offer.pickupLocation.contains(it, ignoreCase = true) }

        // 조건3: 출발지 광역시 AND 도착지 특별시 (예: 인천→서울)
        val condition3 = offer.pickupLocation.contains("광역시", ignoreCase = true) &&
